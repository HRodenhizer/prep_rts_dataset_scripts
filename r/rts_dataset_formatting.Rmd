...
-  Add check for column format, NAs, and reasonable values
-  Add support for creating CentroidLat and CentroidLon columns if they are not present?
...
Before starting, copy your new file into the input_data directory of this rproject.
...
```{r}
check_lat(new_data$CentroidLat)
check_lon(new_data$CentroidLon)
check_region(new_data$RegionName)
check_creator(new_data$CreatorLab)
```

...
-  Remove contribution date when setting seed for UUID
...
```{r}
# If your new file is a geojson, this is a simple vector of the character column names. If your new file is a shapefile, it should be provided as a named list, with the names being the full names and the values being the abbreviated names.
# Geojson Example:
# new_fields <- c('CustomColumn1', 'CustomColumn2')
# Shapefile example:
# new_fields <- c('CustomColumn1' = 'CstmCl1', 'CustomColumn2' = 'CstmCl2')
new_fields <- c('CustomColumn1')
```
  ...
  if (str_detect(new_data_file, '\\.geojson')) {
    new_data <- new_data |>
      select(
        all_of(
          c(!!!required_fields[
            which(!required_fields %in% c('UUID', 'ContributionDate'))
          ]
          )
        ),
        any_of(c(!!!optional_fields)),

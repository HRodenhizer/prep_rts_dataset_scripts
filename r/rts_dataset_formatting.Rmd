...
-  Remove contribution date when setting seed for UID
  ...
  if (str_detect(new_data_file, '\\.geojson')) {
    new_data <- new_data |>
      select(
        all_of(
          c(!!!required_fields[
            which(!required_fields %in% c('UID', 'ContributionDate'))
          ]
          )
        ),
        any_of(c(!!!optional_fields)),
...
```{python}
new_UIDs = [str(UID.UID5(UID.NAMESPACE_DNS, name = seed)) for seed in r.new_seeds]
r.new_UIDs = new_UIDs
```
